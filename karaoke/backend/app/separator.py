...
def separate(
    ...
    if progress_callback:
        progress_callback(0.0, "正在載入人聲分離模型（首次執行需下載，請耐心等候）…")
    ...
    if return_code != 0:
        ...
        raise SeparationError(f"人聲分離失敗（結束碼 {return_code}）：\n{tail}")
    ...
    for stem in DEMUCS_STEMS:
        ...
        if not stem_path.exists():
            raise SeparationError(f"分離完成但找不到 {stem} 音軌，請檢查輸出目錄：{model_dir}")
